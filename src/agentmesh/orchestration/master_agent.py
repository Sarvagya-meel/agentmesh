...
from agentmesh.core.exceptions import AgentRegistryError, ValidationError, WorkflowConflictError
...
from agentmesh.orchestration.planner import CapabilityWorkflowPlanner, WorkflowPlanner
...
class MasterOrchestratorAgent:
    ...
    def _build_graph(self) -> Any:
        ...
        builder.add_edge("prepare_task", "review_task")
        builder.add_conditional_edges(
            "review_task",
            self._decision_route,
            {
    ...
    def _prepare_task(self, state: MasterWorkflowState) -> dict[str, Any]:
        ...
        approval = ApprovalRequest(
            ...
            prompt=f"Approve sending task '{task.name}' to agent '{task.agent_id}'?",
            context={"task": task_payload, "plan_id": str(plan.plan_id)},
        )
        approval_payload = approval.model_dump(mode="json")
        self._emit(state, "TASK_APPROVAL_REQUESTED", {"approval": approval_payload})
        return {"current_task": task_payload, "pending_approval": approval_payload}
