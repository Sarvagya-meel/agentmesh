...
def runtime_factory(kind: str) -> AgentFactory:
    normalized = kind.strip().lower()
    if normalized == "langgraph":
        return create_langgraph_worker_agent
    if normalized == "google-adk":
        return create_google_adk_worker_agent
    raise RuntimeError("AGENT_RUNTIME_KIND must be 'langgraph' or 'google-adk'.")
...
def create_agent_runtime_app(
    *,
    ...
    resolved_kind = kind or os.getenv("AGENT_RUNTIME_KIND", "langgraph")
    resolved_factory = factory or runtime_factory(resolved_kind)
    ...
    async def invoke(body: InvokeRequest, request: Request) -> dict[str, Any]:
        agent = _agent_from(request)
        if body.approval_required and isinstance(agent, ConversationAgent):
