...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    orchestrator = QueuedWorkflowOrchestrator(
        ...
        supervisor_api_url=settings.supervisor_api_url,
        service_token=token,
    ...
    app.state.supervisor_action_service = action_service
...
def create_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(title=f"{settings.app_name} Control Plane", lifespan=lifespan)
