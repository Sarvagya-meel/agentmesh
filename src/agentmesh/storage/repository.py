...
from agentmesh.core.models import AssignmentClaim, Event, EventFilters
...
class InMemoryEventRepository(EventRepository):
    ...
    def append(self, event: Event) -> Event:
        with self._lock:
            ...
            stored = event.model_copy(
                update={"sequence_number": len(workflow_events) + 1},
                deep=True,
            )
    ...
    def query(self, filters: EventFilters) -> list[Event]:
        with self._lock:
            ...
            matches = [event for event in events if self._matches(event, filters)]
            return [event.model_copy(deep=True) for event in matches[: filters.limit]]
    ...
    def list_pending_assignments(self, target_agent: str, *, limit: int = 20) -> list[Event]:
        with self._lock:
            ...
            pending = []
            for event in sorted(assignments, key=lambda item: item.timestamp):
                payload = event.payload if isinstance(event.payload, dict) else {}
...
class PostgresEventRepository(EventRepository):
    ...
    def append(self, event: Event) -> Event:
        with self._connection.transaction(), self._connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT * FROM agentmesh_events WHERE event_id = %s",
                (event.event_id,),
            )
            ...
            cursor.execute(
                ...
                (
                    ...
                    Jsonb(event.payload),
                    event.causation_id,
                    Jsonb([str(item) for item in event.causation_chain]),
        ...
        with self._connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE assignment.event_type = 'TASK_ASSIGNED'
                  ...
                  AND NOT EXISTS (
                    SELECT 1
                    FROM agentmesh_events AS result
        ...
        with self._connection.transaction(), self._connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                INSERT INTO agentmesh_event_claims (
                    event_id, agent_id, worker_id, claim_token, claimed_at,
        ...
        with self._connection.cursor() as cursor:
            cursor.execute(
                ...
                CREATE TABLE IF NOT EXISTS agentmesh_event_claims (
                    event_id UUID PRIMARY KEY REFERENCES agentmesh_events(event_id),
...
def create_event_repository(
    ...
    connection_url = settings.database_url.replace("postgresql+asyncpg://", "postgresql://")
    repository = PostgresEventRepository.from_connection_url(connection_url)
...
def create_claim_repository(
    ...
    if backend != "postgres":
        raise ValidationError("EVENT_STORE_BACKEND must be memory or postgres.")
    connection_url = settings.database_url.replace("postgresql+asyncpg://", "postgresql://")
    repository = PostgresClaimRepository.from_connection_url(connection_url)
    return repository, repository.close
