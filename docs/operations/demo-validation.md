...
- `tests/tools/browser_smoke.cjs`: the Docker-served Streamlit pages in a real browser
...
```powershell
.\.venv\Scripts\python.exe tests\tools\demo_verify.py --reset-data --rounds 2
```
...
documented in `../planning/gap.md`. Main was not rewritten. Additional readiness fixes make
