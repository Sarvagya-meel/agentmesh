...
| `python scripts/test.py` could not find quality tools | System Python has no repository dev dependencies and the wrapper was removed during cleanup | Use the explicit `.venv/Scripts/python.exe -m pytest -q`, `.venv/Scripts/python.exe -m ruff check src tests`, and `.venv/Scripts/python.exe -m mypy --strict src` commands; do not mutate global Python |
