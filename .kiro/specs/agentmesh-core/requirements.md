...
5. WHEN an agent is used as an event source or target, THEN it SHALL be resolved from the runtime registry; validation SHALL NOT depend on a hardcoded list of worker IDs.
...
### Requirement 16: Interview Learning Documentation

**User Story:** As a developer preparing for interviews, I want every major AgentMesh feature to generate technical and non-technical learning notes so that I can explain the project confidently in interviews.

#### Acceptance Criteria

1. WHEN a major spec phase is completed, THEN the project SHALL require a corresponding entry in `docs/learning/INTERVIEW_LEARNING.md` before the phase is considered done.
2. WHEN a learning entry is created, THEN it SHALL include all of the following sections: simple explanation, technical explanation, why it matters, interview short answer (30–45 seconds), interview deep-dive answer (2–3 minutes), business explanation, real AgentMesh example, trade-offs and alternatives, follow-up questions with answer hints, and a resume bullet.
3. WHEN a feature has measurable business impact, THEN the learning entry SHALL include a business-facing explanation written for a non-technical audience.
4. IF a spec phase is marked complete without a corresponding learning entry, THEN the phase SHALL be considered incomplete regardless of test results.
5. WHEN the learning document is updated, THEN it SHALL use the standard feature entry format defined in the Documentation Automation Design section of `design.md`.

---

### Requirement 17: Business Problem Documentation

**User Story:** As a candidate and builder, I want AgentMesh to clearly explain the business problems it solves so that I can present it as a portfolio project and not only a code project.

#### Acceptance Criteria

1. WHEN a new major capability is added to AgentMesh, THEN `docs/business/BUSINESS_PROBLEMS.md` SHALL be updated before the phase is considered complete.
2. WHEN a business documentation entry is created, THEN it SHALL include all of the following: problem statement, current manual process, why that process fails, AgentMesh solution, business impact, example scenario, metrics to track, and a client-facing pitch.
3. WHEN a feature has measurable operational value, THEN the entry SHALL list relevant KPIs such as time saved, reduction in manual follow-ups, auditability, workflow success rate, failure recovery time, and duplicate processing reduction.
4. IF no business problem can be mapped to a feature, THEN the task SHALL include an explicit note explaining why the feature is purely technical infrastructure with no direct business mapping.
5. WHEN business documentation is updated, THEN it SHALL use the standard business problem entry format defined in the Documentation Automation Design section of `design.md`.

---

### Requirement 18: Medium-Ready Content Generation

**User Story:** As a developer building in public, I want every major learning topic to produce Medium-ready content so that I can convert project work into public writing and LinkedIn posts.

#### Acceptance Criteria

1. WHEN a major learning entry is added to `INTERVIEW_LEARNING.md`, THEN a corresponding Medium-ready draft SHALL be created or updated under `docs/content/medium/`.
2. WHEN a Medium draft is created, THEN it SHALL include all of the following sections: catchy title, subtitle, hook introduction, problem statement, why it matters, simple analogy, architecture explanation, small code or pseudocode example where useful, real AgentMesh example, business value, interview learning section, lessons learned, conclusion, LinkedIn-ready short post, and suggested hashtags.
3. WHEN content is generated, THEN the tone SHALL be practical, simple, interesting, and production-focused — written for engineers, recruiters, and hiring managers.
...
12. WHEN `LLM_PROVIDER=groq`, THEN model credentials and model settings SHALL be loaded from the Git-ignored repository-root `.env`; secrets SHALL NOT be stored in tracked files.
13. WHEN automated tests run, THEN they SHALL force `LLM_PROVIDER=mock` before application startup so tests never consume external model quota.

---

### Requirement 20: Governed Master Orchestration

**User Story:** As an operator, I want a master agent to plan work and obtain human approval before both the plan and each worker dispatch so that multi-agent automation remains controlled and auditable.

#### Acceptance Criteria

1. WHEN a workflow starts, THEN the orchestrator SHALL capture the currently online registry agents and create a structured plan containing ordered tasks, dependencies, and target agent IDs.
2. WHEN a plan references an unknown or offline agent, an invalid position, or a forward dependency, THEN the orchestrator SHALL reject the plan before requesting approval.
3. WHEN a valid plan is created, THEN execution SHALL pause until a human approves, revises, or rejects that plan.
4. WHEN a human requests a plan revision, THEN the orchestrator SHALL create a new plan version and request approval again.
5. WHEN a plan is approved, THEN each task SHALL be proposed individually and execution SHALL pause for task approval before emitting `TASK_ASSIGNED`.
6. WHEN a plan or task is rejected, THEN the workflow SHALL be cancelled and no unapproved task SHALL be dispatched.
7. WHEN a task is approved, THEN the orchestrator SHALL emit a directed `TASK_ASSIGNED` event and wait for an external agent result; it SHALL NOT invoke worker implementation code directly.
8. WHEN a worker result is submitted, THEN the task ID SHALL match the active task before the orchestrator advances or fails the workflow.
9. WHEN the process restarts with PostgreSQL event and checkpoint backends enabled, THEN workflow events and LangGraph execution state SHALL remain recoverable.
10. WHEN workflow state is requested, THEN it SHALL be derived deterministically from the append-only event history.
11. WHEN an LLM planner is enabled, THEN it SHALL return a strict structured draft and SHALL NOT generate workflow IDs, task IDs, approval state, or event metadata.
12. WHEN an LLM plan draft is converted to a workflow plan, THEN AgentMesh SHALL generate domain identifiers and validate task ordering, backward-only dependencies, registered agent IDs, and advertised capabilities before requesting approval.
13. IF the configured model provider is unavailable, rate-limited, or returns an unusable response, THEN the API SHALL return a provider error without dispatching a task.
