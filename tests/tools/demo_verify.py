...
def main() -> int:
    ...
    for round_number in range(1, args.rounds + 1):
        ...
        run("graphs", [python, "tests/tools/export_langgraph_mermaid.py", "--check"])
        run(
            "pytest",
        ...
        run(
            ...
            [
                python,
                "tests/tools/system_sanity.py",
                "--no-build",
                "--output-dir",
                str(output / "sanity"),
            ],
        )
        run("browser", [args.node, "tests/tools/browser_smoke.cjs", str(output / "browser")])
